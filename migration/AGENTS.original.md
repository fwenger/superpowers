# Superpowers System (read this first)
...
# Superpowers compatibility rules (Codex)

We use obra/superpowers skills as workflow intent. Some skills were written for Claude Code and may reference tools or capabilities that do not exist in Codex.

When a superpowers skill references a Claude-specific tool/capability, you MUST follow this procedure:

1) Identify the dependency.
   - State exactly which instruction/tool/capability from the skill is not directly available.
...
3) Apply a transparent mapping or fallback.
   ...
   - Claude-specific skill invocation → use Codex skill selection/invocation (e.g., Skills UI / `$` selection), or `superpowers-codex` helper if installed
